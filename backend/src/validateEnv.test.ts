...
describe('Environment & Request Input Configuration Validation', () => {
  ...
  it('rejects invalid URL format in ALLOWED_ORIGINS in production', () => {
    const env = { ...validProdEnv, ALLOWED_ORIGINS: 'not-a-url,https://valid.com' };
    expect(() => validateEnv(env)).toThrow(/contains invalid URL/);
  });

  it('rejects HTTP origins in production (must be HTTPS)', () => {
    const env = { ...validProdEnv, ALLOWED_ORIGINS: 'http://example.com' };
    expect(() => validateEnv(env)).toThrow(/must use HTTPS in production/);
  });

  it('rejects localhost in production', () => {
    const env = { ...validProdEnv, ALLOWED_ORIGINS: 'https://localhost:3000' };
    expect(() => validateEnv(env)).toThrow(/cannot contain localhost/);
  });

  it('rejects 127.0.0.1 in production', () => {
    const env = { ...validProdEnv, ALLOWED_ORIGINS: 'https://127.0.0.1:3000' };
    expect(() => validateEnv(env)).toThrow(/cannot contain localhost/);
  });

  it('rejects raw IP addresses in production (IPv4)', () => {
    const env = { ...validProdEnv, ALLOWED_ORIGINS: 'https://192.168.1.1' };
    expect(() => validateEnv(env)).toThrow(/should use domain names, not IP addresses/);
  });

  it('rejects raw IP addresses in production (IPv6)', () => {
    const env = { ...validProdEnv, ALLOWED_ORIGINS: 'https://[::1]' };
    expect(() => validateEnv(env)).toThrow(/should use domain names, not IP addresses/);
  });

  it('accepts valid HTTPS origins in production', () => {
    const env = { ...validProdEnv, ALLOWED_ORIGINS: 'https://app.example.com,https://admin.example.com' };
    expect(() => validateEnv(env)).not.toThrow();
  });

  it('allows HTTP origins in development mode', () => {
    const devEnv: Record<string, string> = {
      NODE_ENV: 'development',
      ALLOWED_ORIGINS: 'http://localhost:3000,http://192.168.1.1',
    };
    expect(() => validateEnv(devEnv)).not.toThrow();
  });
  ...
  it('rejects malformed URL with missing protocol in production', () => {
    const env = { ...validProdEnv, ALLOWED_ORIGINS: 'example.com' };
    expect(() => validateEnv(env)).toThrow(/contains invalid URL/);
  });
});
